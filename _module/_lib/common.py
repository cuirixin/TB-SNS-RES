...
class Common:
    ...
    def jsOut(data,code=1,error=None):
        return dict(data=data,exception=dict(code=code,error=error))
    ...
    @staticmethod 
    def get_image_path_with_dir(uuid):
        return str(Common.get_image_dir_index(uuid))+"/"+uuid
    
    # D:\_work\_ide\my\tb_user/../resource/uploads/image/1
    @staticmethod 
    def get_image_path(uuid, dir_path=config_base.setting['upload_image']):
        if not str(uuid).isdigit():
            pass
        else:
            dir_path = dir_path+Common.get_image_dir_index(uuid)+'/'
        if not os.path.exists(dir_path):
            os.mkdir(dir_path)
        return dir_path
    
    @staticmethod 
    def get_avator_path(uuid):
        dir_path = config_base.setting['upload_avator']
        if not str(uuid).isdigit():
            pass
        else:
            dir_path = dir_path+Common.get_image_dir_index(uuid)+'/'
    ...
    def generate_short_uuid(length=4):
        ...
        for i in range(length):
            short.append(chars[random.randint(0, 61) ]);
        return ''.join(short)
    ...
    """
    @staticmethod
    def get_point_distance(lata, lona, latb, lonb):
        try:
            lata = float(lata)
            latb = float(latb)
            mlata = 90 - float(lata)
            ...
            mlonb = float(lonb)
            R = 6371004
            C = math.sin(lata / 57.2958) * math.sin(latb / 57.2958) \
                + math.cos(lata / 57.2958) * math.cos(latb / 57.2958) * math.cos((mlona - mlonb) / 57.2958)
        except Exception, e:
            return 0
...
if __name__ == "__main__":
    ...
    #print Common.seconds_to_str(Common.str_to_seconds('April 8, 2015', '%B %d, %Y'), const.Date_Format.DATETIME)
